...
fn main() {
    ...
    match subcommand.as_str() {
        ...
        "generate" => {
            ...
            let outfile = args.remove(0);
            let r = args.remove(0).parse().expect("Failed to parse a number");
            let g = args.remove(0).parse().expect("Failed to parse a number");
            let b = args.remove(0).parse().expect("Failed to parse a number");
